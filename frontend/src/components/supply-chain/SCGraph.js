...
const FOCAL_R = 44;
const PAD = 72;
export function SCGraph({ ticker, legalName, edges, onNodeClick, onHubClick, onFocalClick }) {
    ...
    const onMouseDown = useCallback((e) => {
        if (e.target.closest('g[data-node]'))
            return;
    ...
    const layout = useMemo(() => {
        ...
        const clusters = meta.map(m => {
            ...
            const n = cat.nodes.length;
            const cols = Math.min(COLS, n);
            const chips = cat.nodes.map((node, j) => ({
                ...node,
    ...
    if (!layout)
        return (_jsx("div", { className: "w-full h-full flex items-center justify-center text-terminal-dim text-xs font-mono", children: "No graph data" }));
    const { cx, cy, clusters, vb } = layout;
    return (_jsxs("div", { className: "w-full h-full relative overflow-hidden", style: { minHeight: 240 }, children: [_jsx("button", { onClick: resetView, className: "absolute top-2 right-2 z-10 text-[9px] font-mono tracking-widest px-2 py-1 bg-terminal-surface border border-terminal-border text-terminal-dim hover:text-terminal-text hover:border-terminal-accent/40 rounded-sm transition-colors", children: "RESET" }), _jsx("div", { className: "absolute bottom-2 right-2 z-10 text-[8px] font-mono text-terminal-dim/30 pointer-events-none select-none", children: "pinch to zoom \u00B7 scroll to pan \u00B7 drag to pan" }), _jsxs("svg", { ref: svgRef, viewBox: `${vb.x} ${vb.y} ${vb.w} ${vb.h}`, width: "100%", height: "100%", preserveAspectRatio: "xMidYMid meet", style: { display: 'block', cursor: 'grab', userSelect: 'none' }, onMouseDown: onMouseDown, onMouseMove: onMouseMove, onMouseUp: onMouseUp, onMouseLeave: onMouseUp, children: [_jsxs("defs", { children: [_jsxs("filter", { id: "sc-glow-f", x: "-100%", y: "-100%", width: "300%", height: "300%", children: [_jsx("feGaussianBlur", { in: "SourceGraphic", stdDeviation: "8", result: "b" }), _jsxs("feMerge", { children: [_jsx("feMergeNode", { in: "b" }), _jsx("feMergeNode", { in: "SourceGraphic" })] })] }), _jsxs("filter", { id: "sc-glow-hub", x: "-150%", y: "-150%", width: "400%", height: "400%", children: [_jsx("feGaussianBlur", { in: "SourceGraphic", stdDeviation: "3", result: "b" }), _jsxs("feMerge", { children: [_jsx("feMergeNode", { in: "b" }), _jsx("feMergeNode", { in: "SourceGraphic" })] })] }), _jsxs("filter", { id: "sc-glow-e", x: "-40%", y: "-40%", width: "180%", height: "180%", children: [_jsx("feGaussianBlur", { in: "SourceGraphic", stdDeviation: "1.5", result: "b" }), _jsxs("feMerge", { children: [_jsx("feMergeNode", { in: "b" }), _jsx("feMergeNode", { in: "SourceGraphic" })] })] })] }), _jsxs("g", { transform: `translate(${tf.x},${tf.y}) scale(${tf.s})`, children: [clusters.map(c => {
                                ...
                                return (_jsxs("g", { children: [_jsx("line", { x1: x1, y1: y1, x2: x2, y2: y2, stroke: c.color.edge, strokeWidth: 0.8, strokeOpacity: 0.35 }), _jsx("line", { x1: x3, y1: y3, x2: labelCX, y2: labelCY, stroke: c.color.edge, strokeWidth: 0.6, strokeOpacity: 0.2, strokeDasharray: "3 4" })] }, `spoke-${c.dir}`));
                            }), clusters.map(c => (_jsxs("g", { filter: "url(#sc-glow-hub)", "data-node": "1", style: { cursor: 'pointer' }, onClick: () => onHubClick?.(c.dir, c.label, c.chips), children: [_jsx("circle", { cx: c.hubX, cy: c.hubY, r: HUB_CR + 14, fill: "transparent" }), _jsx("circle", { cx: c.hubX, cy: c.hubY, r: HUB_CR + 4, fill: "none", stroke: c.color.border, strokeWidth: 0.5, strokeOpacity: 0.2 }), _jsx("circle", { cx: c.hubX, cy: c.hubY, r: HUB_CR, fill: c.color.bg, stroke: c.color.border, strokeWidth: 1.2 })] }, `hub-${c.dir}`))), clusters.map(c => (_jsxs("g", { children: [_jsx("rect", { x: c.gridX - 2, y: c.gridY - 2, width: c.gridW + 4, height: c.gridH + 4, rx: 4, fill: "none", stroke: c.color.border, strokeWidth: 0.5, strokeOpacity: 0.15 }), _jsxs("g", { "data-node": "1", style: { cursor: 'pointer' }, onClick: () => onHubClick?.(c.dir, c.label, c.chips), children: [_jsx("rect", { x: c.gridX, y: c.gridY, width: c.gridW, height: LABEL_H, rx: 3, fill: c.color.border + '18' }), _jsx("rect", { x: c.gridX, y: c.gridY, width: c.gridW, height: 2, rx: 1, fill: c.color.border, fillOpacity: 0.6 }), _jsxs("text", { x: c.gridX + c.gridW / 2, y: c.gridY + LABEL_H / 2, textAnchor: "middle", dominantBaseline: "middle", fontSize: 7, fontWeight: "bold", letterSpacing: 1.5, fill: c.color.text, fontFamily: "monospace", children: [c.label, " (", c.total, ")"] })] }), c.chips.map(chip => {
                                        ...
                                        return (_jsxs("g", { "data-node": "1", style: { cursor: 'pointer' }, onMouseEnter: () => setHovered(chip.id), onMouseLeave: () => setHovered(null), onClick: () => onNodeClick(chip), filter: isHov ? 'url(#sc-glow-e)' : undefined, children: [_jsx("rect", { x: chip._x, y: chip._y, width: CHIP_W, height: CHIP_H, rx: 2, fill: isHov ? c.color.bg + 'ff' : c.color.bg + 'dd', stroke: isHov ? c.color.border : c.color.border + '55', strokeWidth: isHov ? 1 : 0.5 }), _jsx("rect", { x: chip._x, y: chip._y, width: 2, height: CHIP_H, rx: 1, fill: accentColor, fillOpacity: 0.85 }), _jsx("text", { x: chip._x + 7, y: nameY, dominantBaseline: "middle", fontSize: 7, fill: isHov ? c.color.text : c.color.text + 'cc', fontFamily: "monospace", fontWeight: "500", children: name }), hasSub && (_jsx("text", { x: chip._x + 7, y: chip._y + CHIP_H - 5, fontSize: 5.5, fill: isAN ? accentColor + 'dd' : c.color.border + '70', fontFamily: "monospace", children: sub.length > 16 ? sub.slice(0, 15) + '…' : sub }))] }, `chip-${chip.id}`));
                                    })] }, `cluster-${c.dir}`))), _jsxs("g", { filter: "url(#sc-glow-f)", "data-node": "1", style: { cursor: 'pointer' }, onClick: () => onFocalClick?.(), children: [_jsx("circle", { cx: cx, cy: cy, r: FOCAL_R + 14, fill: "none", stroke: "#00d4ff", strokeWidth: 0.4, strokeOpacity: 0.1 }), _jsx("circle", { cx: cx, cy: cy, r: FOCAL_R + 5, fill: "none", stroke: "#00d4ff", strokeWidth: 0.6, strokeOpacity: 0.18 }), _jsx("circle", { cx: cx, cy: cy, r: FOCAL_R, fill: "#030c18", stroke: "#00d4ff", strokeWidth: 1.8 }), _jsx("text", { x: cx, y: cy - 6, textAnchor: "middle", dominantBaseline: "middle", fontSize: 16, fontWeight: "bold", fill: "#00d4ff", fontFamily: "monospace", children: ticker }), _jsx("text", { x: cx, y: cy + 11, textAnchor: "middle", fontSize: 6.5, fill: "#4a6070", fontFamily: "monospace", children: legalName.length > 18 ? legalName.slice(0, 17) + '…' : legalName })] })] })] })] }));
}
