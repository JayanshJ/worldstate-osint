...
export function useClusters() {
    ...
    useEffect(() => {
        api.clusters
            ...
            .then(data => {
            setClusters(data.sort((a, b) => new Date(b.last_updated_at ?? 0).getTime() - new Date(a.last_updated_at ?? 0).getTime()));
            setLoading(false);
        })
            .catch(e => {
            setError(String(e));
    ...
    const clearFlash = useCallback((id, key) => {
        setClusters(prev => prev.map(c => c.id === id ? { ...c, [key]: false } : c));
    }, []);
    const mergeClusterUpdate = useCallback((update) => {
        setClusters(prev => {
            const idx = prev.findIndex(c => c.id === update.cluster_id);
            if (idx >= 0) {
                // Update existing — flash "updated"
                const updated = prev.map((c, i) => i === idx
                    ? {
                        ...c,
                        label: update.label,
                    ...
                    : c);
                // Re-sort by recency
                updated.sort((a, b) => new Date(b.last_updated_at ?? 0).getTime() - new Date(a.last_updated_at ?? 0).getTime());
                const timer = flashTimers.current.get(update.cluster_id);
                if (timer)
                    clearTimeout(timer);
                flashTimers.current.set(update.cluster_id, setTimeout(() => clearFlash(update.cluster_id, 'isUpdated'), NEW_FLASH_MS));
                ...
                flashTimers.current.set(update.cluster_id, setTimeout(() => clearFlash(update.cluster_id, 'isNew'), NEW_FLASH_MS));
