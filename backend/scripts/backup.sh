...
FILENAME="${BACKUP_DIR}/worldstate_${TIMESTAMP}.dump"
...
if [ -n "${BACKUP_S3_BUCKET:-}" ]; then
  ...
  aws s3 cp "${FILENAME}.sha256" "s3://${BACKUP_S3_BUCKET}/$(basename "${FILENAME}.sha256")" \
    --storage-class STANDARD_IA
  echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] S3 upload complete"
fi

# ── Prune old local backups ─────────────────────────────────────────────────
find "$BACKUP_DIR" -name "worldstate_*.dump" -mtime "+${RETENTION_DAYS}" -delete
find "$BACKUP_DIR" -name "worldstate_*.dump.sha256" -mtime "+${RETENTION_DAYS}" -delete
